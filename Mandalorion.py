...
parser.add_argument('-u', '--upstream_buffer', type=str, default='10',
                    help='Defines upstream leniency window for polyA and TSS determination (default 10)')
parser.add_argument('-d', '--downstream_buffer', type=str, default='50',
                    help='Defines downstream leniency window for polyA and TSS determination (default 50)')
parser.add_argument('-s', '--subsample_consensus', type=str, default='500',
                    help='Defines how many random subreads are used to polish isoform consensus sequences (default 500)')
...
parser.add_argument('-r', '--minimum_ratio', type=str, default='0.01',
...
parser.add_argument('-I', '--minimum_isoform_length', type=str, default='500',
                    help='Minimum length in nt of isoforms that will be considered')
parser.add_argument('-n', '--minimum_feature_count', type=str, default='2',
                    help='Features (starts,ends, new splice sites) will be considered if they are present in this number of reads')
parser.add_argument('-w', '--splice_site_window', type=str, default='1',
                    help='reads spliced within this number of nucleotides on each side of a splice site will be considered spliced at this site')
parser.add_argument('-A', '--Acutoff', type=str, default='0.5',
                    help='Isoforms with A content of more than this cutoff in a 30nt window surrounding their polyA site will be discarded')
...
print('%s -G 400k --secondary=no -ax splice:hq -t %s %s %s > %s ' %(minimap2, minimap2_threads, genome_sequence, fasta_string, sam_file))
os.system('%s -G 400k --secondary=no -ax splice:hq -t %s %s %s > %s ' %(minimap2, minimap2_threads, genome_sequence, fasta_string, sam_file))
...
os.system('python3 spliceSites.py %s %s %s %s %s %s %s %s' %(clean_psl_file, path, '0.05', genome_annotation, 'g', sam_file,window,feature_count))
print('Identifying Isoforms')
os.system('python3 defineAndQuantifyIsoforms.py %s %s %s %s %s %s %s' %(clean_psl_file, path, downstream_buffer, upstream_buffer, subreads, fasta_files,feature_count)) # This script sort raw reads into isoform bins. The two number variables determine the window around TSS and TES in which read ends can fall and still be matched to the site.
os.system('python3 createConsensi.py -p %s -s %s -c %s -n %s' %(path, subsample_consensus, config_file, minimap2_threads))
os.system('python3 filterIsoforms.py -p %s -i %s -r %s -R %s -n %s -a %s -G %s -c %s -O %s -t %s -e %s -A %s -s %s -d %s -I %s 2> %s' %(path, path+'/Isoform_Consensi.fasta', minimum_ratio, minimum_reads, minimum_internal_ratio, adapter, genome_sequence, config_file, overhangs, minimap2_threads, ends,Acutoff,window,downstream_buffer,minimum_isoform_length,path+'/filter_reasons.txt'))
