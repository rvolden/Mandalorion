...
def configReader(configIn):
    ...
    possible = set(['minimap2', 'consensus', 'racon', 'blat', 'emtrey','poa','medaka'])
    inConfig = set()
...
if args['config']:
    ...
    minimap2, racon, emtrey, blat,poa,medaka = 'minimap2', 'racon', 'emtrey', 'blat','poa','medaka_consensus'
...
def determine_consensus(name, fasta, fastq,counter):
    ...
    for read,sequence in fasta_read_dict.items():
        fasta_reads.append((read,sequence))
    ...
    if len(fastq_reads) > 0:
        ...
        os.system('%s --secondary=no -ax map-ont \
                  ...
                  %(minimap2, out_First, out_F, Poa_Dir,temp_folder+'/'+counter))
        direction_dict={}
        ...
        pairwise = temp_folder + '/'+counter+'_prelim_consensus.fasta'
        ...
        for i in np.arange(1, 2):
            try:
                ...
                os.system('%s --secondary=no -ax map-ont \
                          %s %s > %s 2> %s_minimap2_messages.txt' \
                          %(minimap2, input_cons, out_Fq, overlap,temp_folder+ '/'+counter))

                os.system('%s -q 5 -t 1 --no-trimming\
                          %s %s %s >%s  2>%s_racon_messages.txt' \
                          %(racon, out_Fq, overlap, input_cons, output_cons,temp_folder+ '/'+counter))

                final = output_cons
            except:
                pass
        os.system('mkdir '+temp_folder+ '/'+counter)
#        os.system('%s -f -i %s -d %s -o %s -m bonito_medaka_0.0.9 2> ./medaka_messages.txt' %(medaka,out_Fq,final,temp_folder+ '/'+counter))

        os.system('%s -f -i %s -d %s -o %s > %s_medaka_messages.txt 2>&1' %(medaka,out_Fq,final,temp_folder+ '/'+counter,temp_folder+ '/'+counter))
